...
export const techNotes = {
  ...
  'Remote Attestation': '由硬件签名确认：指定代码和模型确实运行在未被篡改的隔离环境中。',
  'Proof of Intelligence': 'PoI 只记录真实任务产生的有效计算：结果要通过验收，执行回执要完整，重复提交和自造任务不算。',
  zkML: '用零知识证明确认固定模型确实对承诺输入完成了指定推理，同时不公开模型参数或原始输入。',
  VRF: '可验证随机函数。任何人都能核对抽签结果正确，但在结果公布前无法预测谁会被选中。',
  'BFT / HotStuff': '联盟节点通过多方投票确认同一账本状态。即使少量节点故障或作恶，账本仍能完成最终确认。',
} as const;
