...
const runtimeNodes: RuntimeNode[] = [
  {
    ...
    inside: '验证签名 Manifest 与 Schema，解析参与方、事件、工具类型和验收谓词，生成不可变执行 IR。',
    output: '不可变 Contract Instance',
    boundary: '只固定执行语义，不负责模型推理与身份注册。',
  ...
  },
  {
    ...
    icon: 'bolt',
    input: '事件、计时器、上一步状态根',
    inside: '消费签名事件并推进长期状态；等待、超时、重试和人工审批都会生成新的状态根。',
    output: '下一可运行步骤 + State Root',
    boundary: '计算合约状态，不替代联盟链共识。',
    terms: ['IVC', 'Folding'],
  },
  {
    code: '03',
    ...
    inside: '按 list → describe → dry-run → execute 渐进发现企业 API，并把短期授权绑定到工具、参数和时限。',
    output: 'Capability Token + Signed Tool Receipt',
    boundary: '不持有万能密钥，也不枚举无关 API。',
    terms: ['UCAN / ZCAP'],
  },
  {
    code: '04',
    ...
    inside: '检查身份、范围、预算、副作用与风险信号；高风险动作经 FROST 门限授权后才进入状态转移。',
    output: '策略决定 + Threshold Receipt',
    boundary: 'AnySentry 判断风险，ACVM 强制执行决定。',
  ...
  },
  {
    ...
    inside: '聚合状态承诺、工具回执与控制决定，构造结算凭证、长期证明和链上公共输入。',
    output: 'Receipt Root + Proof',
    boundary: '链上不保存业务原文、Prompt 与内部台账。',
