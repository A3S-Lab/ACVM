...
const workflowNotes = {
  ...
  taskKey: ['taskKey · 防重放键', '把规则承诺 C、输出根、裁决根、Worker 身份和任务类别组合成唯一消费键。', 'C 已包含 chainId、Runtime 版本、taskId 和 nonce；键进入 Spent 集合后，结果不能再次结算或生成 ValidPoI。'],
  ...
  PoI: ['PoI · 智能证明体系', 'PoI Worker 提供链下模型推理，验收通过后生成 ValidPoI。', 'ValidPoI 进入有界权重计算；PoI 不等同于 Worker、凭证或 BFT 终局中的任意单一环节。'],
