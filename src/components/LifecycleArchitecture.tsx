...
const lifecycleSteps: LifecycleStep[] = [
  {
    ...
    title: '部署合约',
    actor: '提交业务目录',
    detail: '开发者提交包含 Schema、Worker、Validator 和结算代码的完整目录；运行配置由系统固定。',
    output: 'Contract Tree Root',
    icon: 'terminal',
    column: 1,
    row: 1,
  },
  {
    ...
    title: 'P2P 广播',
    actor: '传播合约包',
    detail: '内容寻址的合约目录先在节点间传播；每个节点独立校验目录根、签名和协议版本。',
  ...
  },
  {
    ...
    title: '激活合约',
    actor: '2f+1 确认',
    detail: '验证者对同一个目录根和初始状态投票，达到 2f+1 后合约正式可用。',
  ...
  },
  {
    ...
    title: 'Worker 执行',
    actor: 'a3s-box 工作负载',
    detail: 'a3s-box 把调用写成固定输入文件；Worker 读取输入，把证据、产物和结果写回任务目录。',
    output: 'Worker Tree Root',
    icon: 'bolt',
    column: 5,
    row: 1,
  },
  ...
  },
  {
    ...
    title: 'Validator 验收',
    actor: 'a3s-box 工作负载',
    detail: '独立 a3s-box 只读同一输入与 Worker 输出，重新核验后写入固定的裁决文件。',
    output: 'Validator Tree Root',
    icon: 'brain',
    column: 4,
    row: 2,
    network: true,
  ...
  },
  {
    ...
    title: '写入账本',
    actor: '区块 · 状态 · 回执',
    detail: '新区块写入状态根、任务文件根、回执根和 PoI；原始敏感文件不进入链上。',
